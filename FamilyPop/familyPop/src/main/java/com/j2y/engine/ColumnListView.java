...
public class ColumnListView extends AdapterView<ListAdapter> {
    ...
    private void readAttrs(final Context context, final AttributeSet attrs) {
        ...
        mOverscroll = false;//attributes.getBoolean(R.styleable.ColumnListView_overscroll, true);
        mFlingDamping = 0;//attributes.getFloat(R.styleable.ColumnListView_fling_damping, 1.5f);
        mSnapSpring = 0;//attributes.getInt(R.styleable.ColumnListView_snap_spring, 100);
        mSnapDamping = 0;//2 * FloatMath.sqrt(mSnapSpring);
        mRubberbandFactor = 0.0f;//attributes.getFloat(R.styleable.ColumnListView_rubberband_factor, 0.4f);
